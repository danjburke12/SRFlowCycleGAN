...
class Split3d(nn.Module):
    ...
    def forward(self, input, logdet=0., reverse=False, eps_std=None, eps=None, ft=None, y_onehot=None):
        # For single channel case (num_channels_consume = 0), we pass through without splitting
        # but still maintain consistent return format
        if self.num_channels_consume == 0:
            if not reverse:
                # Pass through input but create dummy eps tensor of correct shape
                dummy_eps = torch.zeros_like(input)
                return input, logdet, dummy_eps
            else:
                # In reverse, we just pass through
                return input, logdet
        ...
        if not reverse:
            ...
            if mean is None or logs is None:
                # If prior returns None, handle like single channel case
                dummy_eps = torch.zeros_like(input)
                return input, logdet, dummy_eps
