...
class FlowUpsamplerNet3D(nn.Module):
    """3D version of FlowUpsamplerNet for volumetric flow-based super-resolution."""
    ...
    def __init__(self, image_shape, hidden_channels, K, L=None,
        ...
        self.opt = opt
        D, H, W, self.C = image_shape
        self.check_image_shape()

        # Level naming adapted for 3D - using minimum spatial dimension for level calculation
        min_spatial = min(D, H, W)
        if opt['scale'] == 16:
            self.levelToName = {
                0: 'fea_up16',
                1: 'fea_up8', 
        ...
        for level in range(1, self.L + 1):
            # 1. Squeeze
            D, H, W = self.arch_squeeze(D, H, W)

            # 2. K FlowStep  
            self.arch_additionalFlowAffine(D, H, LU_decomposed, W, actnorm_scale, hidden_channels, opt)
            self.arch_FlowStep(D, H, self.K[level], LU_decomposed, W, actnorm_scale, affineInCh, flow_coupling,
                               flow_permutation,
                               hidden_channels, normOpt, opt, opt_get,
                               n_conditinal_channels=conditional_channels[level])
            # Split
        ...
        self.H = H
        self.W = W
        # Use minimum spatial dimension for scale calculation
        min_spatial = min(D, H, W)
        base_size = 160  # Base reference size
        self.scaleD = base_size / D
        self.scaleH = base_size / H  
        self.scaleW = base_size / W
    ...
    def get_n_rrdb_channels(self, opt, opt_get):
        ...
        if blocks is not None:
            ...
            # Use the actual RRDB network's feature dimension
            n_rrdb = opt_get(opt, ['network_G', 'nf'], 64)
        print(f"DEBUG get_n_rrdb_channels (3D): returning {n_rrdb}")
        return n_rrdb

    def arch_FlowStep(self, D, H, K, LU_decomposed, W, actnorm_scale, affineInCh, flow_coupling, flow_permutation,
                      hidden_channels, normOpt, opt, opt_get, n_conditinal_channels=None):
        condAff = self.get_condAffSetting(opt, opt_get)
        if condAff is not None:
            condAff['in_channels_rrdb'] = n_conditinal_channels
        ...
        for k in range(K):
            # Use minimum spatial dimension for position naming
            min_spatial = min(D, H, W)
            position_name = get_position_name(min_spatial, self.opt['scale'])
            if normOpt: normOpt['position'] = position_name
    ...
    def arch_split(self, D, H, W, L, levels, opt, opt_get):
        correct_splits = opt_get(opt, ['network_G', 'flow', 'split', 'correct_splits'], False)
        correction = 0 if correct_splits else 1
        if opt_get(opt, ['network_G', 'flow', 'split', 'enable']) and L < levels - correction:
            logs_eps = opt_get(opt, ['network_G', 'flow', 'split', 'logs_eps']) or 0
    ...
    def arch_additionalFlowAffine(self, D, H, LU_decomposed, W, actnorm_scale, hidden_channels, opt):
        if 'additionalFlowNoAffine' in opt['network_G']['flow']:
            n_additionalFlowNoAffine = int(opt['network_G']['flow']['additionalFlowNoAffine'])
            for _ in range(n_additionalFlowNoAffine):
                self.layers.append(
    ...
    def arch_squeeze(self, D, H, W):
        self.C, D, H, W = self.C * 8, D // 2, H // 2, W // 2  # factor^3 = 8 for 3D squeeze
        self.layers.append(flow.SqueezeLayer(factor=2))
        self.output_shapes.append([-1, self.C, D, H, W])
        return D, H, W
...
def get_position_name_3d(D, H, W, scale):
    """Get position name for 3D volumes using minimum spatial dimension."""
    min_spatial = min(D, H, W)
    downscale_factor = 160 // min_spatial
    position_name = 'fea_up{}'.format(scale / downscale_factor)
    return position_name
